...
export default function Category() {
  ...
  const { data:getAllCategories } = trpc.product.getAllCategory.useQuery();
  const { data:getAllProducts } = trpc.product.getOnePage.useQuery();
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
              {getAllProducts?.map((product) => (
                <Link key={Number(product.id)} href={`/product/${product.id}`} className="group">
